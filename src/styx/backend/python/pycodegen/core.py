...
from styx.backend.python.pycodegen.utils import enquote, ensure_endswith, escape_backslash, linebreak_paragraph
...
class PyFunc(PyGen):
    ...
    def generate(self) -> LineBuffer:
        ...
        if self.docstring_body:
            docstring_linebroken = linebreak_paragraph(escape_backslash(self.docstring_body), width=80 - 4)
        else:
        ...
        buf.extend(
            indent([
                ...
                *(["Returns:", *indent([f"{escape_backslash(self.return_descr)}"])] if self.return_descr else []),
                '"""',
            ])
        )
        ...
        if not self.is_named_tuple:
                ...
                *indent([
                    *(
                        [
                            '"""',
                            *linebreak_paragraph(
                                escape_backslash(self.docstring), width=80 - 4, first_line_width=80 - 4
                            ),
                            '"""',
                        ]
                        if self.docstring
            ...
            if self.docstring:
                buf.extend(
                    indent([
                        '"""',
                        f"{escape_backslash(self.docstring)}",
                        '"""',
                        *args,
                        *blank_before(methods),
                    ])
...
class PyModule(PyGen):
    ...
    def generate(self) -> LineBuffer:
        ...
        return blank_after([
            *(['"""', *linebreak_paragraph(escape_backslash(self.docstr)), '"""'] if self.docstr else []),
            *comment([
